 ...
 * 
 */
public class FiveCardEvaluatorCacheLoader extends CacheLoader<List<Card>, EvaluatedHand> {

	private static final int EXPECTED_NUMBER_OF_CARDS_IN_HAND = 5;

	private final Ordering<Entry<Rank>> byCountThenRank;
	private final LinkedList<Rank> ascendingSortedDistinctRanks;

	public FiveCardEvaluatorCacheLoader() {
		byCountThenRank = new CountThenRankOrdering();
		ascendingSortedDistinctRanks = new LinkedList<>();
	}
	...
	private EvaluatedHand evaluateNewHand(final List<Card> cardsInHand) {
		...
		ascendingSortedDistinctRanks.clear();
